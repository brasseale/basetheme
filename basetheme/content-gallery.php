...
<article id="post-<?php the_ID(); ?>" class="<?php hybrid_entry_class(); ?>">
	...
	<div class="entry-summary">
		...
		<?php $count = basetheme_get_image_attachment_count(); ?>
			
			<p class="image-count">
			
			<?php $count = apply_filters( 'basetheme_gallery_count', sprintf( _n( 'This gallery contains %s image. ', 'This gallery contains %s images. ', $count, 'basetheme' ), $count ) ); 
			
				$date = apply_filters( 'basetheme_gallery_more', sprintf( '<a href="%1$s" title="%2$s" rel="bookmark">%3$s</a>',
					    esc_url( get_permalink() ),
					    esc_attr( get_the_title() ),
					    __( 'View All Images &raquo; ', 'basetheme' )
					)
				);
